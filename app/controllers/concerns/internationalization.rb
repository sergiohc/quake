  ...
  included do # rubocop:disable Metrics/BlockLength
    ...
    def parse_header(header)
      header.gsub(/\s+/, "").split(",").map do |language_tag|
        ...
        [ locale, quality ]
      end.reject do |(locale, quality)| # rubocop:disable Style/MultilineBlockChain
        locale == "*" || quality.zero?
      end.sort_by do |(_, quality)| # rubocop:disable Style/MultilineBlockChain
        quality
      end.map(&:first)
    end

    def detect_from_available(locales)
      locales.reverse.find do |l|
